...
export async function POST(request: NextRequest) {
  try {
    ...
    const result: CommissionCalculationData = {
      ...
      /**
       * EL CSV CRUDO YA NO SE GUARDA, Y NO ES UN OLVIDO.
       *
       * Aquí iba `originalCsv: csvContent`: el fichero de la Tax Document
       * Library ENTERO, dentro de `commission_reports.data`. Ese fichero lleva,
       * fila a fila, la ciudad, el código postal y el país de entrega de cada
       * pedido, el Order ID y un enlace a la factura del comprador. O sea datos
       * personales de compradores, que es justo lo que la spec firmada con
       * Amazon dice que no guardamos.
       *
       * Y estaba a la vista: la política de la migración 011 dejaba leer esa
       * tabla entera con la clave anónima, que viaja en el JavaScript de la web.
