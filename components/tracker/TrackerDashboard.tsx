...
export function TrackerDashboard({ employees }: TrackerDashboardProps) {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Obtener TODOS los logs de estos reportes (sin filtrar por fecha)
      ...
      // Agrupar logs por reporte - mostrar TODOS los reportes con sus logs
      const reportsWithLogs: TrackerReport[] = allReportsData
        .map(report => ({
          ...report,
          logs: (allLogsData || []).filter(log => log.report_id === report.id)
        }))
